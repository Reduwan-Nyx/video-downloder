...
=======
        ...
        # If the user specified a container (mp4/webm/mkv) and not 'auto', prefer it properly.
        if format_type and format_type != 'auto':
            # Replace any 'bestvideo' or 'bestaudio' with ext filter where appropriate
            # Build a safer preference that inserts [ext=<format_type>] where meaningful
            selected_format = selected_format.replace('best[ext=', 'best[')  # normalize
            # For combined video+audio patterns, apply ext filter to each best occurrence
            selected_format = selected_format.replace('best', f'bestaudio[ext={format_type}]/best') if 'bestaudio' in selected_format and 'bestvideo' not in selected_format else selected_format
            # More conservatively, append an ext-preferring alternative
...
=======
                    ...
                    # Replace complex format with a more permissive one
                    simple_video_opts['format'] = 'bestvideo+bestaudio/best'
                    # Remove any ext-specific patterns that may be invalid
                    try:
                        simple_video_opts.pop('postprocessors', None)
                    except Exception:
                        pass
